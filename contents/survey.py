...
def survey_page_1():
    ...
    for i, (qkey, question) in enumerate(questions.items()):
        if qkey in select_responses.keys():
            ...
            response[question] = st.selectbox(f"**{question}**", ["", *select_responses[qkey]], key=f"select_{i}", label_visibility="collapsed")
        else:
            st.write(f"> **{qkey}. {question}**")
...
def survey_page_2():
    ...
    slider_labels = [
        "1 (Does not desribe me at all)", "2", "3", "4", "5", "6", "7 (Describes me very well)"
    ]
...
def survey_page_4():
    ...
    response[questions[1]] = st.selectbox(f"11. **{questions[1]}**", options_11, key="select_11", label_visibility="collapsed")
